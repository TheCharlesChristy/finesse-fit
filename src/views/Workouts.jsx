...
import { Copy, Dumbbell, Footprints, ListChecks, MapPinned, Pencil, Play, Plus, Repeat, Trash2, Trophy, X } from 'lucide-react';
import { Banner, CardTitle, EmptyState, OverflowMenu, SearchInput } from '../components/ui.jsx';
...
function RouteThumb({ path }) {
  ...
  if (!d) return null;
  return <svg className="route-thumb" viewBox="0 0 72 48" aria-hidden="true"><path d={d} /></svg>;
}
  ...
  return (
    ...
    {sessionName && (
      <Banner
        tone="accent"
        icon={Play}
        title={`${sessionName} is in progress`}
        actions={<button className="btn-primary btn-sm" type="button" onClick={onOpenSession}>Open</button>}
      >
    ...
    <div className="layout-split">
      <div className="layout-main">
        <section className="card panel stack">
          <CardTitle icon={ListChecks} action={<button className="btn-ghost" type="button" onClick={onNewPlan}><Plus size={16} aria-hidden="true" /> New plan</button>}>Start a workout</CardTitle>
          <div className="quick-starts">
            <button type="button" className="action-tile primary" onClick={() => onQuickStart('run')}>
              <span className="action-tile-icon"><Footprints size={22} aria-hidden="true" /></span>
            ...
            <button type="button" className="action-tile" onClick={() => onQuickStart('empty')}>
              <span className="action-tile-icon"><Dumbbell size={22} aria-hidden="true" /></span>
          ...
          {plans.length ? (
              ...
              {plans.map((plan) => (
                <div key={plan.id} className="list-row plan-row">
                  <button type="button" className="plan-row-main" onClick={() => onEditPlan(plan)}>
                    ...
                    <span className="plan-row-meta">{planMeta(plan)}{planHasGps(plan.blocks) && <span className="badge"><MapPinned size={11} aria-hidden="true" /> GPS</span>}</span>
                  </button>
                  <button type="button" className="btn-primary btn-sm plan-start" onClick={() => onStartPlan(plan)} aria-label={`Start ${plan.name}`}><Play size={16} aria-hidden="true" /> <span>Start</span></button>
            ...
            <EmptyState icon={ListChecks} title="No plans yet" action={<button className="btn-secondary" type="button" onClick={onNewPlan}><Plus size={18} aria-hidden="true" /> Design a workout</button>}>
              Design a workout once — sets, circuits, runs or intervals — then start it in one tap.
            </EmptyState>
          )}
        </section>
        ...
        <section className="card panel stack">
          ...
          <div className="list">
                  ...
                  <button type="button" className="session-main" onClick={() => (isActivity ? onOpenActivity(workout) : onEditWorkout(workout))}>
                    ...
                    {cardio.length > 0 && (
                      <span className="session-line session-cardio">
                        <span className="truncate"><Footprints size={13} aria-hidden="true" /> {activityLabel(cardio[0].activity)}{workout.distance > 0 ? ` · ${fmtDistance(workout.distance, units)}` : ''}</span>
            ...
            {!workouts.length && (
              ...
              >
                Start a plan or a free run, or log a past session — Progress lights up straight away.
              </EmptyState>
            )}
          </div>
        </section>
      ...
      <div className="layout-aside">
        <section className="card panel stack">
          <CardTitle icon={MapPinned} action={<button className="btn-ghost" type="button" onClick={onPlanRoute}><Plus size={16} aria-hidden="true" /> Plan a route</button>}>Routes</CardTitle>
          {routes.length ? (
            <div className="list">
              {routes.map((route) => (
            ...
            <EmptyState icon={MapPinned} title="No routes yet" action={<button className="btn-secondary" type="button" onClick={onPlanRoute}><Plus size={18} aria-hidden="true" /> Plan a route</button>}>
              Draw a loop on the map, then run it with the route on screen.
            </EmptyState>
          )}
        </section>
